...
#"CYP3A5_Frequencies_2025", this brings gene frequency data for each of the ethnic groups, values come from PharmGKB database
#Each column represents an ethnic group, each row represents a gene variant, values in the table are the gene frequency
#removes NA values replaces with 0, and names the rows after gene variant names
Cyp3A5_Frequencies<- read_excel("CYP3A5_Frequencies_2025.xlsx")
Cyp3A5_Frequencies<- as.data.frame(Cyp3A5_Frequencies)
Cyp3A5_Frequencies[is.na(Cyp3A5_Frequencies)]<- 0
rownames(Cyp3A5_Frequencies)<- Cyp3A5_Frequencies$`CYP3A5 allele`
...
rownames(Cyp3A5_Tests)<- Cyp3A5_Frequencies$`CYP3A5 allele`
...
Cyp3A5_Phenotypes<- read_excel("Cyp3A5_Diplotypes_2025.xlsx")
...
#takes the same Phenotype table "Cyp3A5_Phenotypes" described above with the same columns, creates a new table called "Cyp3A5_Phenotypes_Test" and makes additional adjustments
...
#For each calculation of clinical sensitivity, the Test Name has to be replaced in the column title of the Cyp3A5_Tests
...
#Creates an addtional column for "Cyp3A5_Phenotypes_Test" called "Test"
...
colnames(Cyp3A5_Phenotypes_Test)<- c("A1.1", "A2.1", "Phenotype", "EHR", "Multiply", "Test")
...
rownames(TestSensitivity)<- c("African American/Afro-Caribbean", "American", "Central/South Asian", "East Asian", "European", "Latino", "Near Eastern", "Oceanian", "Sub-Saharan African")
...
#Calculations already performed, this is only exporting the values
#Cyp3A5_Counts<- as.data.frame(read_excel("Cyp3A5Tests.xlsx", sheet= "Sheet2"))

#table(Cyp3A5_Phenotypes_Test$A1)

# Make a copy of the phenotype table
#Cyp3A5_Phenotypes_Test <- Cyp3A5_Phenotypes
#TestSensitivity
